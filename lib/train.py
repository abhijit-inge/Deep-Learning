...
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
	...
	for epoch in range(max_epochs):
		...
		for iter in range(iter_start, iter_end):
			...
			#############################################################################
			loss = None
			forward1 = model.forward(data_batch, True)
			loss = loss_func.forward(forward1, labels_batch)
			#print('Current Loss is: ', loss)
			dLoss = loss_func.backward()
			backward1 = model.backward(dLoss)
			#grads = model.net.grads.copy()
			optimizer.step()
			#print(grads)
			#c += 1
			#print(c)
		...
		#############################################################################
		#print('Before  :', model.net.grads['fc2_b'], grads['fc2_b'])
		train_acc = compute_acc(model, data["data_train"][0], data["data_train"][1])
		val_acc = compute_acc(model, data["data_val"][0], data["data_val"][1])
		#############################################################################
		#                             END OF YOUR CODE                              #
		#############################################################################
		...
		if val_acc > opt_val_acc:
			...
			#                             END OF YOUR CODE                              #
			#############################################################################
		#print('After  :', model.net.grads['fc2_b'], grads['fc2_b'])
